...
# 创建平滑插值
spline = UnivariateSpline(r, tg, s=0.05)  # s 控制平滑度
r_smooth = np.linspace(min(r), max(r), 500)
tg_smooth = spline(r_smooth)

plt.figure(figsize=(8, 6))

# 绘制平滑数据
plt.plot(r_smooth, tg_smooth, label='Free Energy')

# 标出 y 轴最高点
max_y = np.max(tg_smooth)
max_x = r_smooth[np.argmax(tg_smooth)]
