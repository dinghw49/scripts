...
if '__main__' == __name__:
    ...
    plt.xlim(0, np.max(x))
    plt.ylim(np.min(y)-2, np.max(y)+0.5)
    plt.minorticks_on()

    if args.title:
        plt.title(args.title)

    output_file = args.output if args.output.endswith('.svg') else args.output + '.svg'

    logger.info("Saving to {}".format(output_file))
    plt.savefig(output_file, dpi=args.dpi, format='svg')
